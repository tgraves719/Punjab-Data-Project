...
let net=null;
function sizeNet(){
 /* Desktop: full-bleed — break out of main's 1200px column. Small screens: fit
    the viewport exactly (no negative margin, no horizontal overflow) and take a
    shorter box so the controls stay reachable. Buffer stays 1:1 with CSS px, so
    cpos()'s client→buffer mapping is exact on both. */
 const cv=$('nwC'),colW=cv.parentElement.clientWidth||1140,small=window.innerWidth<760;
 cv.width=small?Math.max(280,colW):Math.max(colW,Math.min(window.innerWidth-70,1900));
 cv.height=small?Math.max(380,Math.min(window.innerHeight-230,720))
                :Math.max(640,Math.min(window.innerHeight-250,1050));
 cv.style.maxWidth='none';cv.style.marginLeft=Math.min(0,(colW-cv.width)/2)+'px';
}
function initNet(){if(!net){
 sizeNet();net=makeNet($('nwC'));
 $('nwMin').oninput=()=>{$('nwMinV').textContent=$('nwMin').value;net.rebuild();};
 $('nwType').onchange=()=>net.rebuild();
 // re-fit on rotate/resize (only while the tab is showing)
 let rt=null;addEventListener('resize',()=>{if(!$('nw').classList.contains('on'))return;
  clearTimeout(rt);rt=setTimeout(()=>{sizeNet();net.refit();},250);});}}
function makeNet(cv){
 ...
 function rebuild(){
  ...
  $('nwLegend').innerHTML=NTYPES[type].legend.map(([t,c])=>
   ...
   ' · node size = entries · scroll or pinch to zoom · drag the background to pan · drag a node to move it · tap/click to highlight';
  loop();
 }
 ...
 /* pad: hit slop in screen px — fingers are blunter than cursors */
 function at(wx,wy,pad){for(let i=ns.length-1;i>=0;i--){const n=ns[i];
  const dx=wx-n.x,dy=wy-n.y,rr=(r(n)+(pad||4))/view.s;
 ...
 /* Pointer Events — one path for mouse, touch and pen. Two fingers pinch-zoom
    and pan together; one finger drags a node or the background; tap toggles the
    highlight and shows the node's stats (there is no hover on touch). The
    canvas sets touch-action:none, so the browser never steals these gestures —
    which is exactly why the old mouse-only handlers left mobile inert. */
 const ptrs=new Map();let pinch=null;
 const slop=ev=>ev.pointerType==='mouse'?4:14;
 const mid=()=>{const[a,b]=[...ptrs.values()];
  return[(a[0]+b[0])/2,(a[1]+b[1])/2,Math.hypot(a[0]-b[0],a[1]-b[1])||1];};
 function showTip(n,cx,cy){const t=$('tip');t.style.display='block';
  t.style.left=Math.min(cx+14,window.innerWidth-290)+'px';t.style.top=(cy+10)+'px';
  t.innerHTML='<b>'+esc(n.label)+'</b><br>'+n.type+' · '+n.n+' entries · '+
   fmt(n.cop)+' copies · '+n.deg+' connections';}
 cv.onpointerdown=ev=>{
  // capture keeps a drag alive if the finger leaves the canvas; never let a
  // rejected capture (odd pointer states, synthetic events) kill the gesture
  try{cv.setPointerCapture(ev.pointerId)}catch(e){}
  ptrs.set(ev.pointerId,cpos(ev));
  if(ptrs.size===2){const[mx,my,d]=mid();pinch={mx,my,d};drag=null;pan=null;
   $('tip').style.display='none';return;}
  if(ptrs.size>2)return;
  const[wx,wy]=world(ev);drag=at(wx,wy,slop(ev));
  if(drag){alpha=Math.max(alpha,0.12);loop();}
  else{const[mx,my]=cpos(ev);pan={mx,my,ox:view.ox,oy:view.oy};
   cv.style.cursor='grabbing';$('tip').style.display='none';}};
 cv.onpointermove=ev=>{
  if(ptrs.has(ev.pointerId))ptrs.set(ev.pointerId,cpos(ev));
  if(pinch&&ptrs.size>=2){const[mx,my,d]=mid();
   zoomAt(mx,my,d/pinch.d);                    // scale about the pinch centre
   view.ox+=mx-pinch.mx;view.oy+=my-pinch.my;  // and track two-finger drag
   pinch={mx,my,d};draw();return;}
  ...
  if(hover)showTip(hover,ev.clientX,ev.clientY);else $('tip').style.display='none';
  if(!raf)draw();};
 const endPtr=ev=>{
  ptrs.delete(ev.pointerId);if(ptrs.size<2)pinch=null;
  if(pan){pan=null;cv.style.cursor='grab';}
  if(drag){const[wx,wy]=world(ev);const n=at(wx,wy,slop(ev));
   if(n===drag)sel=(sel===drag?null:drag);
   if(ev.pointerType!=='mouse'){                // touch has no hover: tap = stats
    if(sel)showTip(sel,ev.clientX,ev.clientY);else $('tip').style.display='none';}
   drag=null;alpha=Math.max(alpha,0.25);loop();}};
 cv.onpointerup=endPtr;cv.onpointercancel=endPtr;
 cv.onpointerleave=ev=>{if(ev.pointerType==='mouse'){
  $('tip').style.display='none';hover=null;if(!raf)draw();}};
 ...
 rebuild();return{rebuild,refit(){autofit=true;loop();}};
}
/* ---------- market ---------- */
(function(){
 const M=%%MARKET%%;
