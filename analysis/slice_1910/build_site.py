...
<section id="me">
 ...
 <p><b>Uncertainty.</b> %%FLAGGED%% items sit in the four quarters' adjudication queues (degraded digits, margin-cut copyright numbers, pencil-obscured serials, plus cross-entry checks such as registration collisions) — per-entry flags are visible in the detail panel and filterable ("flagged only"). None are silent guesses.</p>
 <p><b>Scope.</b> Registered print runs measure publisher supply decisions under a legal-deposit regime — not readership, not literacy. Self-publication (%%SELFPUB%% of entries) is reported as a statistic, not forced into the network.</p>
 <p><b>Prior-hand marginalia.</b> The bound volumes carry pencil annotations — X-marks, a running margin count, "Specimen" notes, and handwritten multi-part verso indexes (Q1, Q3, Q4) — in an earlier, unidentified hand that <i>predates</i> Prof. Davis's acquisition of the volumes (confirmed with him). We capture them per-entry (the "annotated" filter) as a genuine second-order layer over the register; their provenance and meaning are an open question, not a claim about Davis.</p>
 </div>
</section>
</main>
...
let net=null;
function initNet(){if(!net){net=makeNet($('nwC'));
...
function makeNet(cv){
 ...
 function step(){
  ...
  for(let i=0;i<ns.length;i++){const a=ns[i];
   for(let j=i+1;j<ns.length;j++){const b=ns[j];
    ...
    if(d2<250000){const f=1500*k/d2;dx*=f;dy*=f;a.vx+=dx;a.vy+=dy;b.vx-=dx;b.vy-=dy;}}}
  ...
  for(let i=0;i<ns.length;i++)for(let j=i+1;j<ns.length;j++){
   ...
   const d=Math.max(0.5,Math.sqrt(dx*dx+dy*dy)),minD=(r(a)+r(b))/Math.max(view.s,0.5)+3;
   if(d<minD){const push=(minD-d)/d*0.45;dx*=push;dy*=push;
    if(a!==drag){a.x-=dx;a.y-=dy}if(b!==drag){b.x+=dx;b.y+=dy}}}
 }
 function fit(){
  ...
  s=Math.max(0.25,Math.min(1.2,s));
 ...
 function loop(){cancelAnimationFrame(raf);
  (function frame(){
   if(alpha>0.02||drag){for(let i=0;i<2;i++)step();if(!drag)alpha*=0.985;}
